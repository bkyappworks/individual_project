...
def search_jobs(position,country):
    ...
    print('this is a checkpint: len(search_bars)',len(search_bars))
    ...
    all.extend(data)     
    return all
# search_jobs('Data Engineer','United States')
...
def saveDB(data):
    ...
    sql = "INSERT IGNORE INTO job_raw (job_id,position,company,location,hiringstatus,posttime,savetime,url,description) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    back = cursor.executemany(sql, data) 
    connection.commit()
    print('Items save to db: ',back)
    finish = time.perf_counter()
    print(f'Finished in {round(finish-start, 2)} second(s)') 
saveDB(search_jobs('Data Engineer','United States'))  # Software Engineer, Data Scientist, Fronend Engineer, Backend Engineer, Data Analyst, Data Engineer
print('Done!')
